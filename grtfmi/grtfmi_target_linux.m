function grtfmi_target_linux(grtfmi_dir, modelName)
    usewsl = get_param(modelName, 'WSLOpt');
    if(strcmp('on',usewsl))
        grtfmi_compile_wsl(grtfmi_dir, modelName)
    else
        ...
        rootpath = fullfile(msysroot, 'x86_64-linux');
        make = fullfile(msysroot, 'usr/bin/make.exe');
        gcc = fullfile(rootpath, 'bin/linux-gcc.exe');
        gpp = fullfile(rootpath, 'bin/linux-g++.exe');
        ...
        fprintf(fid, 'CMAKE_FIND_ROOT_PATH:STRING=%s\n', strrep(msysroot, '\', '/'));
        fprintf(fid, 'CMAKE_FIND_ROOT_PATH_MODE_LIBRARY:STRING=%s\n', 'ONLY');
        fprintf(fid, 'CMAKE_FIND_ROOT_PATH_MODE_INCLUDE:STRING=%s\n', 'ONLY');
